# Blackbird 🖤✈️ — *(working title)*

> An open-source, local-first video editor. Codename **Blackbird**, after the SR-71.

The SR-71 Blackbird captured its footage at Mach 3 — higher and faster than anything could reach. This is an editor in that spirit: bring your material in, cut it clean, and get out — all on your own machine.

Editing video comes down to a handful of moves you repeat all the time: tight cuts, punchy on-screen subtitles, clean audio. It should work just as well for **vertical** clips (Shorts, Reels) as for **regular landscape** video. Today most people pay a monthly subscription to a web tool for exactly those few things. Blackbird is the opposite: **free, local, and yours**.

## Why I'm building this

I'm a content creator, and honestly? I'm tired of paying a monthly subscription for every single tool. Editing, subtitles, audio cleanup — everything is SaaS now, and the bills pile up. It's exhausting, and it hits hardest exactly when you're a **small creator** who can't really afford to spend.

So Blackbird exists for two reasons:

1. **To have something free for myself** — my own editor, no subscription, running on my own machine.
2. **To give back to open source** — sharing it so other creators can edit **completely free**, and together we can stop renting the basics.

The rest follows from that:

- The core edits are simple and repetitive — they shouldn't cost a recurring fee.
- Makers should **own** the tools they use every day.
- **Local-first:** your footage never leaves your machine.
- Editing software can feel **calm and precise**, not cluttered and noisy.

## Status

**Very early.** We're designing the *interface first*. This repo currently holds **static UI prototypes** — HTML only, not functional yet — to explore different visual directions before a single feature is built.

## Take a look 👀

Open the prototypes and tell us which direction feels right. Each is one screen, one mood:

| Direction | Feel | Open |
|---|---|---|
| **Aurora** | soft, glassy, colorful | [`prototypes/01-aurora.html`](prototypes/01-aurora.html) |
| **Minimal** | quiet, restrained, lots of air | [`prototypes/02-minimal.html`](prototypes/02-minimal.html) |
| **Terminal** | monospace, TUI-inspired | [`prototypes/03-terminal.html`](prototypes/03-terminal.html) |
| **Studio** | dense, pro, tool-first | [`prototypes/04-studio.html`](prototypes/04-studio.html) |

> 🌐 View them live from the [project page](https://zabahd4k.github.io/blackbird-editor/).

## Feedback

This is a work in progress and the look is **not decided yet** — that's the whole point of the prototypes. Open an issue or drop a note with the direction you like (or mix ideas from several). Everything here is up for discussion.

---

*Made by [Alejandro Zabaleta](https://github.com/ZabaHD4K). Design exploration — expect things to change.*
